...
public class AddingPicturesFragment extends Fragment implements AddingPicturesAdapter.Listener {
    ...
    private void configureOnClickRecyclerView(){
        ItemClickSupport.addTo(binding.fragmentAddingpicturesRecyclerView, R.layout.fragment_addingpictures_item)
                .setOnItemClickListener((recyclerView, position, v) -> {
                    //todo
                    String picture = mAdapter.getPicture(position);
                    Toast.makeText(getContext(), "You clicked on user : "+ picture, Toast.LENGTH_SHORT).show();
    ...
    private void updateUI(List<String> propertyPictureList){
    ...
    public void onClickDeleteButton(int position) {
        List<String> pictureList = mViewModel.propertyPictureListMutableLiveData.getValue();
